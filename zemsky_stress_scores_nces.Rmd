...
load_charges <- function(year)
{
  ...
  df <-
    ...
    rename_all(tolower) %>%
    mutate(in_state_tuition = as.numeric(tuition2),
           out_state_tuition = as.numeric(tuition3)) %>%
    select(unitid, in_state_tuition, out_state_tuition)
}
...
system.time({
  ...
  institutions <-
    ...
    filter(distance_ed == 0,
           ...
           deggrant == '1',
           ft_ug == '1') %>%
    select(-`Fiscal Year`, -distance_ed, -obereg, -opeflag, -deggrant, -ft_ug) # %>%
...
metrics <-
  ...
  inner_join(charges_data, by = c('unitid', 'Fiscal Year')) %>%
