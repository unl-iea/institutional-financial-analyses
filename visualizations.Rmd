...
ggplot(data = metrics[Unitid == params$unitid],
  ...
  geom_line(aes(y = `NMarket Price`,
                color = 'Market Price'),
            size = 1) +
  geom_line(aes(y = `NUG First-time First Year Enrollment`,
                color = 'FY Enrollment'),
            size = 1) +
  geom_line(aes(y = `NRetention`,
                color = 'Retention'),
            size = 1) +
  geom_line(aes(y = `NState Appropriations`,
                color = 'Appropriations'),
            size = 1) +
...
label <- data.table(
    `UG First-time First Year Enrollment` = c(alert_line$Alert, warn_line$Warning),
    `Fiscal Year` = '2010-2011',
    label = c("Alert", "Warning")
  )
...
ggplot(data = df,
  ...
  # Add line labels
  geom_text(aes(label = label), data = label, hjust = "left") +
  # set y-axis to comma format
  scale_y_continuous(labels = scales::label_comma()) +
  labs(title = 'First-year Enrollment',
       subtitle = inst)
...
label <- data.table(
    Retention = c(alert_line$Alert, warn_line$Warning),
    `Fiscal Year` = '2010-2011',
    label = c("Alert", "Warning")
  )
...
label <- data.table(
    `Market Price` = c(alert_line$Alert, warn_line$Warning),
    `Fiscal Year` = '2010-2011',
    label = c("Alert", "Warning")
  )
...
if (sector == 'Public') {
  ...
  label <- data.table(
      `State Appropriations` = c(alert_line$Alert, warn_line$Warning),
      `Fiscal Year` = '2010-2011',
      label = c("Alert", "Warning")
    )
  ...
  label <- data.table(
      `Endowment/Expenses` = c(alert_line$Alert, warn_line$Warning),
      `Fiscal Year` = '2010-2011',
      label = c("Alert", "Warning")
    )
...
```
```{r}
metrics[Unitid == params$unitid][order(year_key)] # [, .(ref = first(`Endowment/Expenses`))]
```
